...
Client (legacy migration script)
        ...
        │  Headers: Authorization: Bearer <JWT>, X-Contract-Id: <contract>
        │  Body:    { "batch": [...] }
        ▼
┌─────────────────────────────────────────────────────┐
│                  Hyperf HTTP Server                  │
│                  (Swoole, port 9501)                 │
│                                                      │
│  ApiTokenMiddleware  ──── JWT validation             │
...
POST /api/v1/migration/companies
  │
  ├─ 1. ApiTokenMiddleware: decode JWT → set contract_id, user_id
...
| `TokenService` | JWT generation/validation utilities used by `ApiTokenMiddleware` | — |
...
- **JWT (HS256)**: Every request requires `Authorization: Bearer <token>` signed with `JWT_SECRET`
- **Contract isolation**: Token's `contract_id` claim must match `X-Contract-Id` header; mismatches return 403
